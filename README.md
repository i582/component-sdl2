...
Класс `Component` имеет метод `setText` для установки текста. Добавим текст недавно созданному компоненту \(рассмотрим только метод`setup` окна `MyWindow`\):
...
Все стили для текста можно найти  в [разделе ](vse-podderzhivaemye-css-stili.md)`css`. 
...
Добавим прослушиватель события клика на наш компонент, используя лямбда-функцию для более краткой записи:
