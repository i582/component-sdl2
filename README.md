...
Данное поведение можно предотвратить  добавим для родительского компонента стиль `overflow: hidden`, тогда все дочерние компоненты будут обрезаться по размерам родительского.
...
В нем прекрасно просматривается вложенность одних компонентов в другие.
