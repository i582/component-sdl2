...
string Kit::font_find::find()
{
    ...
    for (auto& path : paths_with_style)
    {
        ...
        if (filename_without_ext.size() - _font_name.size() < 3)
        {
            ...
            const string& _font_modifiers = filename_without_ext.substr(_font_name.size(), filename_without_ext.size() -
                                                                                           _font_name.size());
    ...
    if (_font_weight_found[_needle_font_weight] == false)
    {
        for (size_t i = _needle_font_weight; i < _count_font_weights; i++)
        {
            if (_font_weight_found[i] == true)
            {
                ...
                // cache
                _cache.push_back(
                        font_cache(_font_name, _font_style, _font_weight, _font_weight_found_path[i].generic_string()));
    ...
    {
        // cache
        _cache.push_back(font_cache(_font_name, _font_style, _font_weight,
                                    _font_weight_found_path[_needle_font_weight].generic_string()));
...
Kit::font_cache::font_cache(const string& _font_name, font_style _font_style, font_weight _font_weight,
                            const string& _path)
{
