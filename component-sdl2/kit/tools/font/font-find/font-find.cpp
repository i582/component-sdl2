...
Kit::font_find::font_find(string font_name, font_style font_style, font_weight font_weight)
{
...
string Kit::font_find::find()
{
	...
	auto it = std::find(_cache.begin(), _cache.end(), needle_font);
	if (it != _cache.end())
	{
		return it->path();
	}
	...
	string relative_font_directory = "\\Windows\\Fonts";
	...
	for (auto& p : fs::recursive_directory_iterator(current_path))
	{
		...
		{
			string path = to_lower(p.path().filename().generic_string());
	...
	case Kit::font_style::NORMAL:
	{
		for (auto& path : paths)
		{
			string filename = to_lower(path.filename().generic_string());
			string filename_without_ext = to_lower(path.filename().stem().generic_string());
	...
	for (auto& path : paths_with_style)
	{
		...
		if (filename_without_ext.size() - _font_name.size() < 3)
		{
			...
			string _font_modifiers = filename_without_ext.substr(_font_name.size(), filename_without_ext.size() - _font_name.size());
...
Kit::font_cache::font_cache(string _font_name, font_style _font_style, font_weight _font_weight, string _path)
{
