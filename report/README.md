...
## Настройка интерфейса, класс `Component`
...
Второй способ, это получить компонент по его идентификатору и вызвать непосредственно у него функцию `setText`. Для этого у окна есть функция `getElementById`:
...
После того, как текст установлен, его нужно стилизовать.
